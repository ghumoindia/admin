...
import {
  ...
  fetchActivities,
  updateActivity,
} from "../../hooks/slice/activitySlice";

export default function Activity() {
  const dispatch = useDispatch();
  const activity = useSelector((store) => store.activity.activities);
  ...
  const handleSubmit = async (e) => {
    ...
    if (editActivity) {
      result = await dispatch(
        updateActivity({ id: editActivity, data: formDataToSend })
      );
      if (result?.payload?.success) {
        ...
        toast.error("Failed to update activity: " + result?.payload?.error);
      }
      setEditActivity(null);
    } else {
      result = await dispatch(addActivity(formDataToSend));
      if (result?.payload?.success) {
        toast.success("Activity added successfully!");
        setShowForm(false);
        getData();
      } else {
        toast.error("Failed to add activity: " + result?.payload?.error);
      }
    }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (result?.payload?.success) {
        ...
        toast.error("Failed to delete activity: " + result.error);
      }
    } catch (error) {
